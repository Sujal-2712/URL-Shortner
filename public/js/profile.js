function toggleVisitHistory(shortId) {
  ...
  historyContainers.forEach(function (historyContainer) {
    ...
    if (historyContainer.id === "visitHistoryDiv_" + shortId || historyContainer.id === "noVisitHistoryDiv_" + shortId) {
...
function deleteURL(obj, shortId, userObjID) {
  ...
  try {
    ...
    req.open("GET", "/user/delete?shortId=" + shortId + "&userid=" + userObjID, true);
    req.send();
...
document.getElementById('myForm').addEventListener('submit', async function (event) {
  ...
  try {
    ...
    if (data.redirect == undefined) {
      document.getElementById('inputURL').value = "Please enter the URL here...!!";
    }
    else {
      document.getElementById('inputURL').value = data.redirect;
      document.getElementById('shortURL').innerHTML = `<a href="${data.redirect}" class="underline"> ${data.url} </a>`;
    }
